...
func writeFloat(dst *Buffer, val float64, fmt byte, prec, bitSize int) {
	var bits uint64
	var flt *floatInfo
	switch bitSize {
	case 32:
...
func bigFtoa(dst *Buffer, prec int, fmt byte, neg bool, mant uint64, exp int, flt *floatInfo) {
	d := new(decimal)
	d.Assign(mant)
...
func formatDigits(dst *Buffer, shortest bool, neg bool, digs decimalSlice, prec int, fmt byte) {
	switch fmt {
	case 'e', 'E':
...
// %e: -d.ddddde±dd
func fmtE(dst *Buffer, neg bool, d decimalSlice, prec int, fmt byte) {
	// sign
	if neg {
		dst.WriteByte('-')
	}
...
// %f: -ddddddd.ddddd
func fmtF(dst *Buffer, neg bool, d decimalSlice, prec int) {
	// sign
	if neg {
		dst.WriteByte('-')
	}
...
// %b: -ddddddddp±ddd
func fmtB(dst *Buffer, neg bool, mant uint64, exp int, flt *floatInfo) {
	// sign
	if neg {
		dst.WriteByte('-')
	}
...
func formatBits(dst *Buffer, u uint64, base int, neg bool) {
